...
public class MultiImageView<P extends MultiImageView.IPhoto> extends LinearLayout {
    public static int MAX_WIDTH = 0;

    // 照片的Url列表
    private List<P> imagesList;
    ...
    private int pxImagePadding = (int) DeviceUtils.dpToPixel(getContext(), 3);// 图片间的间距
    ...
    public void setList(List<P> lists) throws IllegalArgumentException {
        if (lists == null) {
    ...
    private ImageView createImageView(int position, final boolean isMultiImage) {
        P photoInfo = imagesList.get(position);
...
//		Glide.with(getContext()).load(photoInfo.url).diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
        ImageUtils.loadImage(getContext(), photoInfo.url, imageView);
        return imageView;
    }
